...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Arrow function
   ...
   arrow f with forEach:
   let names = ['Hannah', 'Anie', 'Anett', 'Lily'];
  ...
  // move down the tetrominos in every second
  ...
  function moveLeft() {
    undraw();
    const isALeftEdge = current.some(index => (currentPosition + index) % GRID_WIDTH == 0);
  ...
  function moveRight() {
    undraw();
    const isARightEdge = current.some(index => (currentPosition + index) % GRID_WIDTH === GRID_WIDTH -1);
